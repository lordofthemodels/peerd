...
const CROSS_MODULE_IMPORT = {
  ...
  regex: '(^|/)peerd-(?!runtime/(?:administrative|browser-authority|controller(?:-administrative|-contributor|-model|-tools|-turn|-turn-semantics)?|kernel-executable|semantic|skills)\\.js$|runtime/(?:memory/store|skills/(?:registry|store)|tools/hooks/registry|transfer/(?:secret-policy|self-sync-surfaces|transfer)|voice/settings)\\.js$|runtime/contacts/(?:store|aggregate)\\.js$|runtime/observability/contributor-store\\.js$|engine/(?:app-assets|artifact|app-manifest|authority)\\.js$|egress/(?:kernel-storage\\.js|confirm/protocol\\.js)$)[a-z]+/(?!index\\.js$|background\\.js$|controller\\.js$|offscreen\\.js$|repository\\.js$|voice-host\\.js$|kernel-(?:browser|credentials|custody|memory|network|transfer|turn|turn-authority)\\.js$|ui\\.js$).+',
